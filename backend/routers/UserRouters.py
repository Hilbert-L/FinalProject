...
@check_token
async def upload_profile_picture(token: str = Depends(verify_user_token), file: UploadFile = File(...)):
    ...
    update_results = users_collections.update_one(filter, {"$set": {"image": data_uri}})
